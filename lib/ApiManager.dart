...
//import 'package:scheduler/model/meetings.dart';
...
class ApiManager {
  Future<LocationModel> getLocationDetails() async {
    ...
    //print('*** token ** $token');
    var response =
        await client.get('https://schedulercons.herokuapp.com/api/v1/country');
  ...
  Future<Meetings> getMeetings(String userId) async {
    ...
    var response = await client.get(
        'https://meetingscheduledata.herokuapp.com/api/v1/user/getuser?id=$userId');
    //   'https://meetingscheduledata.herokuapp.com/api/v1/user/getuser?id=$userId');
  ...
  Future<UMeetings> getUser(String userId) async {
    var client = http.Client();
    var meetingDetails = null;
    //print('*** token ** $token');
    var response = await client.get(
        'https://meetingscheduledata.herokuapp.com/api/v1/user/getuser?$userId');
    // var response = await client.get('https://myconstituency.herokuapp.com/api/v1/meeting');
    print("RESPONSE ${response.statusCode} ; BODY = ${response.body}");
    if (response.statusCode == 200) {
      var jsonString = response.body;
